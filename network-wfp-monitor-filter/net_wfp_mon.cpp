...
)
{
	...
	do
	{
		...
		RtlInitUnicodeString(&nt_device_name, NETWFPMON_NT_DEVICE_NAME);
		...
		RtlInitUnicodeString(&win32_device_name, NETWFPMON_DOS_DEVICE_NAME);
...
NTSTATUS net_wfp_start_filter(_In_ PDRIVER_OBJECT pDriverObject)
{
	...
	status = net_wfp_register_callout(g_wfp_device_object,&NETWFPMON_CALLOUT_GUID);
	DBG_ASSERT(NT_SUCCESS(status));
	if (!NT_SUCCESS(status)) {
		goto __exit;
	}
...
#if defined(DBG)
	DbgPrint("netwfprdr, Driver loaded: time:%s, RegistryPath:%ws\n", __TIME__, pRegistryPath->Buffer);
#endif
...
NTSTATUS net_wfp_register_sublayer()
{
	...
	sublayer.subLayerKey = NETWFPMON_SUBLAYER_GUID;
...
NTSTATUS net_wfp_register_filter()
{
	...
	filter.subLayerKey = NETWFPMON_SUBLAYER_GUID;
	filter.weight.type = FWP_UINT8;
	filter.weight.uint8 = 0xf;		
	filter.numFilterConditions = 0;	
	filter.layerKey = FWPM_LAYER_ALE_AUTH_CONNECT_V4;	
	filter.action.calloutKey = NETWFPMON_CALLOUT_GUID;
